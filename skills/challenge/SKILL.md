...
**Protocol:** Full rules in `agents/shared/CHALLENGER_PROTOCOL.md` — including the Consensus & agreement map for concurrent multi-model runs (Act On / Consider / Noted / Dismissed; single-model runs remain fully valid). `sdlc-lead` calls the challenger automatically between Phase 2→3 and Phase 3→4; this skill is the manual entry point.
