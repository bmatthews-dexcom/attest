...
**Safety rails (ALWAYS enforced):** NEVER force-push to main/release, NEVER `--no-verify`, NEVER commit secrets (scans staged files for .env/keys/tokens), NEVER `git add -A` blindly, NEVER add Claude attribution unless the project's log already uses it, NEVER branch a new unit over a prior unit's dirty tree (Clean-Tree Precondition — commit/branch the prior unit first, never stash-and-carry-forward), ALWAYS save a reflog backup before destructive ops, ALWAYS verify with `git status` + `git log --all --oneline --graph` before AND after, ALWAYS run the post-merge scope-attribution check (`git show --stat`) right after merging.
