...
- Includes: `p/owasp-top-ten`, `p/security-audit`, `p/secrets`, `p/default`, language pack, framework pack, language-native pack (e.g. `p/bandit` for Python, `p/gosec` for Go), IaC packs (if relevant), community rules (Trail of Bits, elttam, GitLab, 0xdea if C/C++), **custom gap-filler rules** (160 rules across 10 languages — loaded from the user's personal store at `~/.config/opencode/.semgrep/custom-rules/` (global) or `.opencode/.semgrep/custom-rules/` (project install), auto-selected per detected language; covers JS/TS, Python, Go, Java, C#, Kotlin, Rust, PHP, Swift, C++), project-specific rules from `.semgrep/project-rules/` inside the audited repo
