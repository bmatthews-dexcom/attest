...
Enforced by `~/.config/opencode/scripts/validators/validate-no-reinvent.sh` (hard-fails edits to `GENERATED_FILES.txt` paths; warns on wholesale rewrites of tracked files). This rule exists because a Mode-4 audit once falsely reported canonical loop-engineering files as "missing" and overwrote 6 of them with inferior stubs — a regression a single `diff` would have prevented.
...
**Gate "done" mechanically too:** before the Completion Manifest, run `bash ~/.config/opencode/scripts/handoff-done.sh <packet-file>`. It checks the verify report is ALL GREEN **and fresher than your last edit**, the tree is committed, commits are pushed, PRODUCE files exist, and any required completion-report section is present. `DONE-CHECK: RED` items are fixed, not argued with; print the completion phrase only on `DONE-CHECK: GREEN`.
